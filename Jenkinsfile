 ...
 * FIX: Wrapped all local 'def FULL_IMAGE_NAME' definitions inside a 'script' block 
 * within the 'withCredentials' block to resolve the last compilation error.
 */
pipeline {
  agent any
  ...
  stages {
    ...
    stage('Build & Deploy: Frontend') {
      ...
      steps {
        script {
          // Set pipeline environment variables (this block is correct)
        ...
        withCredentials([
          ...
          // NOTE: The conflicting 'def FULL_IMAGE_NAME' line has been REMOVED here.

          dir("${env.DIR_NAME}") {
            // FIX: Define FULL_IMAGE_NAME as a shell variable in an executable script block
            script {
                def FULL_IMAGE_NAME = "${DOCKER_USER}/${env.SERVICE_NAME}:${env.IMAGE_TAG}"
                env.FULL_IMAGE_NAME = FULL_IMAGE_NAME // Export for shell access outside this block
            }
